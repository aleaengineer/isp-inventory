@extends('layouts.app')
@section('title', 'Edit User — ISP Inventory')
    ...
    <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-lg max-w-2xl">
        <form action="{{ route('users.update', $user) }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-1">Password <span class="text-slate-500">(kosongkan jika tidak diubah)</span></label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="role" class="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-100 focus:border-cyan-500/50 focus:outline-none transition" required>
                        <option value="operator" {{ $user->role === 'operator' ? 'selected' : '' }} class="bg-slate-800">Operator</option>
                        <option value="admin" {{ $user->role === 'admin' ? 'selected' : '' }} class="bg-slate-800">Admin</option>
                    </select>
                </div>
                <div>
