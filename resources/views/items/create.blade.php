@extends('layouts.app')
@section('title', 'Create Item — ISP Inventory')
        ...
        <form action="{{ route('items.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="serial_number" value="{{ old('serial_number') }}" class="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-100 focus:border-cyan-500/50 focus:outline-none transition" required>
                    @error('serial_number') <p class="text-rose-400 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-1">MAC Address</label>
                    <input type="text" name="mac_address" value="{{ old('mac_address') }}" class="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-100 focus:border-cyan-500/50 focus:outline-none transition">
                    @error('mac_address') <p class="text-rose-400 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div>
                    ...
                    <input type="number" name="stock_quantity" value="{{ old('stock_quantity', 0) }}" min="0" class="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-100 focus:border-cyan-500/50 focus:outline-none transition" required>
                </div>
                <div>
                    ...
                    <select name="unit" class="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-100 focus:border-cyan-500/50 focus:outline-none transition">
                        <option value="pcs" class="bg-slate-800">Pcs</option>
                        <option value="roll" class="bg-slate-800">Roll</option>
                        <option value="meter" class="bg-slate-800">Meter</option>
                        <option value="set" class="bg-slate-800">Set</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-1">Location</label>
                    <input type="text" name="location" value="{{ old('location') }}" class="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-100 focus:border-cyan-500/50 focus:outline-none transition">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="supplier_id" class="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-100 focus:border-cyan-500/50 focus:outline-none transition">
                    ...
                    @foreach ($suppliers as $s)
                        <option value="{{ $s->id }}" class="bg-slate-800">{{ $s->name }}</option>
                    @endforeach
                </select>
            </div>
